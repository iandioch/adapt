...
def main(corpus_path):
    ...
    if GET_TOP_BIGRAM_PMI:
        ...
        for bigram in bigramcounts:
            ...
            if bigram_freq <= IGNORE_BIGRAM_THRESHOLD:
                continue
        ...
        num_per_line = 4
        num_lines = 25
        print('\n'.join(''.join(map(lambda x: '{:>30} ({:.3f})'.format(x[1], x[0]), pmis[i:num_lines*num_per_line:num_lines])) for i in range(0, num_lines)))
        '''i = 0
        while pmis[i][0] > 10:
            print(pmis[i])
            i += 1'''
